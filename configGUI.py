...
def configure(camdata):
    ...
    if camdata:
        ...
        cam = camdata['camera_name']
        UI = camdata['userid']
        IP = camdata['ipaddress']
        chVal = camdata['channel']
        Ch = camdata['channel']
        psd = "admin123"
        # if ChVal.get() == '':
        #     Ch = "101"
        ...
        ## create video link and pass it to markRoi  function in markRegion.py file ##
        vid_url = "rtsp://" + UI + ':' + psd + '@' + IP + "/Streaming/Channels/"+ Ch
        info = markRoi(vid_url,cam)
...
def quitWindow():
    ...
    try:
        ...
        tkinter.messagebox.showinfo("Error","Close the already open csv and add again!")
